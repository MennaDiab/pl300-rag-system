...
with st.sidebar:
    ...
    # ---- Live stats (moved here from the top of the main page) ----
    st.markdown("### 📊 Stats")
    rt = st.session_state.last_response_time
    s1, s2 = st.columns(2)
    s1.metric("📄 Docs", DOC_COUNT)
    s2.metric("✂️ Chunks", CHUNK_COUNT)
    s3, s4 = st.columns(2)
    s3.metric("🔍 Top-K", k)
    s4.metric("⚡ Time", f"{rt:.2f}s" if rt else "—")
    ...
    <div class="sidebar-card">
        ...
        <div class="row"><span>LLM</span><span>{MODEL_NAME.split('/')[-1]}</span></div>
